...
SJFPreemptive::SJFPreemptive(QObject *parent, std::vector<Process> processes)
    ...
    arrivedQueue([](Process* a, Process* b) {
        return a->getRemainingTime() > b->getRemainingTime(); // Min-heap behavior
    })
{
    ...
    connect(schedulerTimer, &QTimer::timeout, this, &SJFPreemptive::schedule);
...
void SJFPreemptive::schedule() {
    ...
    if (!arrivedQueue.empty() && currentProcess != nullptr) {
        arrivedQueue.push(currentProcess);
        currentProcess = arrivedQueue.top();
        arrivedQueue.pop();
    }
...
void SJFPreemptive::addNewProcess(Process *p)
{
